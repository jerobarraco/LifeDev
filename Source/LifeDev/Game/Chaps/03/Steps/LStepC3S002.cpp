...
ALStepC3S002::ALStepC3S002():Super() {
	...
	RandFB->DelayMax = 2;
	RandFB->ValueMin = -.01;
	RandFB->ValueMax = .005; // you wouldn't think is so easy to get out of a flashback, do you?
	UseFBRand = true;
	UseRain = true;
	FinishItems = {
	...
	IntersHint = { // card already has hint. hint the book (poem has hint), hint the pictures (tape has hint, so does batts, pictures hints the plant)
		...
		TSoftObjectPtr<AInteract>(FSoftObjectPath("/Game/LifeDev/Game/Sys/Game_L.Game_L:PersistentLevel.PuzzleI03_UAID_D8BBC116E501EDD901_1630772030"))
	};
